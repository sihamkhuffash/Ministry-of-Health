...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Query to get total normal disease cases for the selected hospital, month, and year
    $total_normal_result = "
        SELECT i.hospital_ID, h.hospital_name, 
               SUM(`normal_diseas_male` + `normal_diseas_female`) AS total_normal
        FROM `inputs` i
        JOIN `hospitals` h ON i.hospital_ID = h.hospital_ID
        WHERE i.month = ? AND i.year = ? AND i.hospital_ID = ?
        GROUP BY i.hospital_ID, h.hospital_name
    ...
    // Query to get total chronic disease cases for the selected hospital, month, and year
    ...
    // Query to get detailed input data for the selected hospital, month, and year
    $input_data_result = "
        SELECT i.Input_ID, h.hospital_name, i.normal_diseas_male, i.chronic_diseas_male, 
               i.normal_diseas_female, i.chronic_diseas_female
        FROM `inputs` i
        JOIN `hospitals` h ON i.hospital_ID = h.hospital_ID
        WHERE i.month = ? AND i.year = ? AND i.hospital_ID = ?
    ";
